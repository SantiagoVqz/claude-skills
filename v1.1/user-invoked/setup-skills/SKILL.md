...
- **Delivery shape** — whether this repo can carry stacked PRs, so `/phase-done`, `/ship`, and `/cleanup` know which mode they run in
...
Stacked PRs: [enabled / not enabled]. When enabled, `/phase-done` opens each phase as its own stack layer and `/ship` publishes the whole stack; otherwise both run in solo-branch mode.
```
