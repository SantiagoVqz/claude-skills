...
My skill harness for Claude Code. It is the [`mattpocock/skills`](https://github.com/mattpocock/skills) set, kept verbatim under my own names, apart from one added line each in `to-spec` and `setup-skills`. His work is worth a look. Beside it sit my own `worktree`, `dispatch`, `ship`, `cleanup`, `close-ticket`, `setup-worktrees`, `upgrade-skills`, `ticket-clerk`, `ui-review` and `unslop`, plus his in-progress `retro` and `pr`.
