...
My skill harness for Claude Code. It is the [`mattpocock/skills`](https://github.com/mattpocock/skills) set, kept verbatim under my own names. His work is worth a look. Beside it sit my own `dispatch`, `ship`, `cleanup`, `close-ticket`, `ticket-clerk`, `ui-review` and `unslop`, plus his in-progress `retro` and `pr`.
...
My own skills sit on top of that flow without changing it. `/dispatch <spec>` runs `/implement` per ticket in a subagent on one spec branch, `close-ticket` closes each ticket as its commit lands, `/ship` opens the PR with a `/pr` body, and `/cleanup` tears the branch down after the merge. Dispatch state is the five triage labels: `ready-for-agent` is the queue, `ready-for-human` is a checkpoint, `needs-triage` is a ticket the agent failed twice.
