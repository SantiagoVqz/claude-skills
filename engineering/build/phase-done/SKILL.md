...
The per-phase closer inside a multi-phase `/implement`: run it after each phase, then continue to the next — or hand to `/ship` once the last phase is done. It closes **one phase**; the branch-level Standards + Spec review is `/two-axis-review`, run once at the end.
