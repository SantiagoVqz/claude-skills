---
name: ship
description: "Rebase the current ticket branch onto the trunk, run the full test suite, push, and open its PR — run after /implement has committed and reviewed. Never merges."
...
Take a committed, reviewed ticket branch and **land it on the remote as a pull request** — nothing more. `/implement` owns the code, tests, review, and commits; `ship` owns the git that leaves the machine: rebase, verify, push, PR. It never merges the PR — that stays the human's call.
