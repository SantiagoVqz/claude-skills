#!/usr/bin/env bash
#
# provision.sh — provision the git worktree you are standing in.
#
#   scripts/provision.sh            # env files, dependencies, ports, own database
#   scripts/provision.sh --no-db    # the above without the database step
#   scripts/provision.sh db         # the database step on its own
#   scripts/provision.sh db share   # point at the primary's database instead (read-only use)
#   scripts/provision.sh db drop    # remove this worktree's own database
#
# It never creates a worktree; it sets up the one you are in. Idempotent: a
# second run copies nothing, keeps stamped ports, and finds the database there.
# The primary checkout refuses to run it.
#
# One database per worktree, always. The primary's database is the template;
# a worktree gets <APP_DB>_<branch>, so a migration here never touches what the
# primary, or any other worktree, is running.
#
# PORTING: edit the CONFIG block only. Delete the DATABASE block for a repo with
# no database; the engine detects its absence.
set -euo pipefail
...
# 2. Dev-server ports. Final ports are ${BASE}${NN}; NN comes from the branch
#    number, then moves up until the pair is free. Leave both empty for a repo
#    with no dev servers.
FE_PORT_BASE="51"
BE_PORT_BASE="80"

# Stamp the env vars that carry those ports. Runs with $FE_PORT, $BE_PORT,
# $DB_PORT and $WT set.
stamp_ports() {
...
# ============================================================================
# DATABASE — Postgres in a docker compose service. Delete the whole block for
# a repo with no database.
# ============================================================================

APP_DB="myapp"          # the primary's database, the template for every fork
DB_USER="myapp"
DB_PASSWORD="myapp"
DB_PORT="5432"          # the container's published port, shared by every worktree
DB_SERVICE="db"         # compose service; empty runs psql on the host instead

stamp_database() {   # point the app at database $1
  set_env "$WT/backend/.env" DATABASE_URL \
    "postgresql+psycopg://$DB_USER:$DB_PASSWORD@127.0.0.1:$DB_PORT/$1"
  set_env "$WT/backend/.env" TEST_DATABASE_URL \
    "postgresql+psycopg://$DB_USER:$DB_PASSWORD@127.0.0.1:$DB_PORT/${1}_test"
}

migrate() {          # bring database $1 to head
  ( cd "$WT/backend" && DATABASE_URL="postgresql+psycopg://$DB_USER:$DB_PASSWORD@127.0.0.1:$DB_PORT/$1" \
...
# ---- database engine: repo-independent ----
...
share_db() {
  start_db_server
  stamp_database "$APP_DB"; DB_NAME="$APP_DB"
  warn "This worktree shares $APP_DB. A migration here breaks every other checkout."
}
...
# The database step runs by default: isolation you must ask for is isolation
# you forget to ask for.
DB_STEP="fork"
case "${1:-}" in
  "")       ;;
  --no-db)  DB_STEP="skip" ;;
  db)       DB_STEP="${2:-fork}"
            [[ "$DB_STEP" =~ ^(fork|share|drop)$ ]] || die "use db [fork|share|drop]" ;;
  *)        die "unknown argument '$1' — use db [fork|share|drop] or --no-db" ;;
esac
...
DB_NAME=""
if declare -f fork_db >/dev/null; then
  case "$DB_STEP" in
    fork)  fork_db ;;
    share) share_db ;;
    drop)  drop_db ;;
    *)     warn "Skipped the database — run 'scripts/provision.sh db' for it" ;;
  esac
fi
