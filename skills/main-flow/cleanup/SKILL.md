...
You cannot delete a branch that any worktree has checked out. `/dispatch` works in `.claude/worktrees/dispatch`, so park that worktree on the trunk first; the worktree itself stays for the next spec.

```bash
git -C .claude/worktrees/dispatch checkout --detach <trunk> 2>/dev/null || true   # only if the worktree exists
