...
- **Keyed** — the artifact carries this spec's name or path. Yours to destroy.
...
- **Keyed** — the project name contains the branch or spec. Yours end to end. Tear the stack down *before* `git worktree remove`; compose needs its config file to resolve the project:
  ```bash
...
Report before removing — this is machine-global, not spec-scoped, and a dangling volume can still hold data if its container was removed without it.
