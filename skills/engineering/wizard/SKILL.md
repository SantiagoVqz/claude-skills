...
A wizard is always **scratch** — one run, one throwaway file, living entirely outside the repo. Write it to the session scratchpad, or to `mktemp -d` when there is none. The repo it configures stays untouched by the script itself; if the user wants one checked in, hand them the temp path and let them move it.
...
- Give the user the absolute scratch path and the command to run it.
